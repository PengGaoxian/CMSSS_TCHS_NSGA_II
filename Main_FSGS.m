% 功能：基于 NSGA-II 框架求解云制造服务组合调度问题（FSGS 策略）。
%       采用双层目标：上层为 Provider 侧预热能耗最小化，下层为 Demander 侧 QoS 综合最优。
%       与 Main_MEOS 的核心差异：调度采用纯正向策略（scheduling_Makespan），
%       以最早可用时间为准分配服务窗口，不做反向遍历优化，用于与 MEOS 对比实验。
%
...
%      b. 物流计算（logistics）→ 正向调度（scheduling_Makespan）→ 物流时段提取
...
%   5. 保存全部工作区变量至 Data_FSGS.mat
%
% 输出：（保存至 Data_FSGS.mat，供 paint_Data.m 加载绘图）
...
save(fullfile(outDir, 'Data_FSGS.mat'));
