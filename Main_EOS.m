% 功能：基于 NSGA-II 框架求解云制造服务组合调度问题（EOS 策略）。
%       采用双层目标：上层为 Provider 侧预热能耗最小化，下层为 Demander 侧 QoS 综合最优。
%       调度采用纯能耗优化的正向遍历策略（scheduling_Energy），
%       将服务执行窗口贴近空闲时段右端以最大化后向衔接度，不保证 makespan 不变。
%
...
%      b. 物流计算（logistics）→ 纯能耗调度（scheduling_Energy）→ 物流时段提取
...
%   5. 保存全部工作区变量至 Data_EOS.mat
%
% 输出：（保存至 Data_EOS.mat，供 paint_Data.m 加载绘图）
...
while(gen <= gen_max)
    ...
    [Start_candidate_service,End_candidate_service] = scheduling_Energy(Population_combined,Th,Tc,Ts,Idle,ordertime,Time_required_max,Tl);
...
save(fullfile(outDir, 'Data_EOS.mat'));
