% 功能：为种群中每个个体计算各子任务的服务开始/结束时间（FSGS-E 策略）。
%       第一阶段正向调度确定 makespan，第二阶段从前向后正向遍历，
%       在不延误后续子任务的前提下将服务窗口向衔接度更高的位置移动，以降低预热能耗。
%       与 scheduling_Makespan_Energy 不同之处仅在第二阶段方向：
%         正向（本文件）：处理子任务 i 时，下一子任务开始时间尚未被调整，约束更保守；
%         反向（scheduling_Makespan_Energy）：处理子任务 i 时，后续子任务已被调整，约束更灵活。
%
% 输入：
...
%   【第二阶段：正向遍历优化能耗（从第 1 到第 subtask_num-1 个子任务）】
...
function [Start_candidate_service,End_candidate_service] = scheduling_Makespan_Energy_Forward(Population,Th,Tc,Ds,Idle,ordertime,Time_required_max,Dl)
...
for k = 1:population_size
    ...
    for i = 1:subtask_num
        ...
        while true
            ...
            if end_time > deadline
                ...
                break;
            elseif isempty(start_time_next_index)
                Start_candidate_service(k,i) = start_time;
                End_candidate_service(k,i) = end_time;
                break;
            elseif mod(start_time_next_index,2)==1
                start_time = Periods(start_time_next_index);
            elseif mod(start_time_next_index,2) == 0
                if start_time_next_index ~= end_time_prev_index+1
        ...
        start_time = end_time + logistics_duration;
    end

    %% 第二阶段：正向遍历优化能耗（不延误后续子任务）
    for i = 1:subtask_num-1
        ...
        if cohesion < 1
            if Periods(start_time_next_index)+logistics_duration <= start_time_next_subtask
                end_time = Periods(start_time_next_index);
                start_time = end_time-service_duration;
            else
                end_time_new = start_time_next_subtask-logistics_duration;
